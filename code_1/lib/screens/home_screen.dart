...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
            ...
            child: Column(
              ...
              children: [
                ...
                FadeTransition(
                  opacity: _fadeAnimation,
                  child: Column(
                    // Wrap title and list in a Column
                    children: [
                      const Text(
                ...
                FadeTransition(
                  opacity: _fadeAnimation,
                  child:
                      const UserStoriesWidget(), // Assuming this widget CAN be const
                ),
                const SizedBox(height: 120),
                FadeTransition(
                  opacity: _fadeAnimation,
                  child:
                      const ExplanationWidget(), // Assuming this widget CAN be const
                ),
                const SizedBox(height: 120),
                FadeTransition(
                  opacity: _fadeAnimation,
                ...
                FadeTransition(
                  ...
                  child: Wrap(
                    ...
                    children: [
                      ...
                      Flexible(
                        child:
                            SocialWidget(), // Left Widget (SocialWidget) - likely not const
                      ),
                      Flexible(
                        child:
                            ContactInfoWidget(), // Middle Widget (ContactInfoWidget) - likely not const
                      ),
                      Flexible(
                        child:
                            TeamWidget(), // Right Widget (TeamWidget) - likely not const
                      ),
                    ],
                  ),
                ),
  ...
  Widget _buildVolunteerList() {
    return StreamBuilder<List<Volunteer>>(
      stream:
          _volunteerService
              .watchVolunteers(), // Get the stream from the service
      builder: (context, snapshot) {
        // --- Check for errors FIRST ---
        if (snapshot.hasError) {
          print(
            "StreamBuilder Error (Volunteers): ${snapshot.error}",
        ...
        if (!snapshot.hasData || snapshot.data!.isEmpty) {
          print(
            "StreamBuilder (Volunteers): No data received. Check Firestore data and rules.",
          );
